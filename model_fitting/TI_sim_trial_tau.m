function [perf, traces] = TI_sim_trial_tau(param)
...
% 072023 SPLA EDITED TAU TO NOW BE DIFFERENT FOR X AND DELTA MODELS
% ACCORDING TO FITTED TAU TO META-d' OF REAL SUBJECTS' DATA
%
% INPUT
% -----
%
...
perf.rt2        = -1;
perf.responded  = 0;
% perf.responded2 = 0; %SPLA 072023
perf.responded2_x = 0; %SPLA 072023
perf.responded2_delta = 0; %SPLA 072023

% initialize traces
if param.save_traces
    traces.x_it     = [x_i,     nan(2, param.tmax-1)];
    traces.delta_it = [delta_i, nan(2, param.tmax-1)];
    traces.s = zeros(2,1); %SPLA
else
    traces.x_it     = [];
    traces.delta_it = [];
end
...
t  = 2;
while t <= param.tmax && ~(perf.responded && perf.responded2_x && param.exit_on_response)
...
%             perf.conf_Cd   = delta_i(perf.resp+1); SPLA COMMENTED OUT FOR
%             X VS DELTA TAU 072023
%             perf.conf_Cd_i = delta_i;

%SPLA
%             doing this to run both x and delta taus with just one
%             response 072023
            perf.conf_Cd   = traces.delta_it(perf.resp+1,t2-31);
            perf.conf_Cd_i = traces.delta_it(:,t2-31);

            perf.rt2_x        = t;
            perf.responded2_x = 1;

            perf.rt2_delta    = t-31;
            perf.responded2_delta = 1;
        end
